...
export default function Register({ }: Props) {
  ...
  const handleRegisterSubmit = async (formData: { email: string, password: string }) => {
    try {
      const res = await fetch("127.0.0.1:5000/api/v1/auth/register", {
        method: "POST",
        credentials: "include",
        headers: {
    ...
    <Container maxW="lg" py={{ base: '12', md: '24' }} px={{ base: '0', sm: '8' }}>
      ...
      <Stack spacing="6">
        <form onSubmit={handleFormSubmit}>
          ...
          <Stack spacing="6">
            <Button isLoading={loading} type='submit' colorScheme='green'>Sign up</Button>
            <HStack>
              <Divider />
